...
parser.add_argument('--lr_fix_step', type=int, default=10, help='learning rate step for fix_step')
parser.add_argument('--lr_custom_step', type=list, default=[20,30,40], help='learning rate steps for custom_step')
parser.add_argument('--lr_decay', type=float, default=0.1, help='decay for every epoch')
...
def train(training_loader, optimizer, model, criterion, epoch, args):
    ...
    for i, (input, target) in enumerate(training_loader):
        ...
        if i % args.print_freq == 0:
            logging.info("===> Epoch[{}]({}/{}): Loss: {:.6f}, Batch time: {:.4f} / {:.4f}, Data time: {:.4f} / {:.4f}"
...
def validate(val_loader, model, criterion, args, epoch=0):
    ...
    with torch.no_grad():
        ...
        for i, (input, target) in enumerate(val_loader):
            ...
            im_gt_y = im_gt_y[0,:,:]
            im_b_y = im_b_y[0,:,:]
            psnr = utils.PSNR(im_gt_y, im_b_y, shave_border=args.upscale_factor)
            ...
            psnr = utils.PSNR(im_gt_y, im_h_y, shave_border=args.upscale_factor)
